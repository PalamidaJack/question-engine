...
from qe.models.genome import Blueprint, ModelPreference, CapabilityDeclaration
from qe.runtime.router import AutoRouter, TIER_MODELS
...
def test_autorouter_select_first_model_in_tier():
    """With empty journal and no env overrides: select() returns the first model in the tier's list."""
...
def test_context_manager_build_messages_starts_with_system():
    """build_messages returns a list starting with a system message containing the blueprint's system_prompt."""
    blueprint = Blueprint(
...
def test_context_manager_truncation_never_drops_system():
    """Phase 0 truncation: when history is over token limit, oldest messages are dropped first, system prompt is never dropped."""
    blueprint = Blueprint(
        service_id="test-service",
        display_name="Test",
        version="1.0",
