...
class ModelMarketStore:
    ...
    async def add_or_update_model(
        ...
        await self._db.execute(
            ...
            (provider, model_id, model_name, context_length, capabilities, rate_limit_rpm, rate_limit_rpd, last_checked, updated_at)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
            ON CONFLICT(provider, model_id) DO UPDATE SET
                ...
                is_available = 1
            """,
            (provider, model_id, model_name, context_length, capabilities, rate_limit_rpm, rate_limit_rpd, now, now),
        )
        await self._db.commit()
    ...
    async def add_or_update_provider(
        ...
        await self._db.execute(
            ...
            (provider, api_base, requires_api_key, rate_limit_default_rpm, rate_limit_default_rpd, notes, last_scraped)
            VALUES (?, ?, ?, ?, ?, ?, ?)
            ON CONFLICT(provider) DO UPDATE SET
                ...
                is_active = 1
            """,
            (provider, api_base, 1 if requires_api_key else 0, rate_limit_default_rpm, rate_limit_default_rpd, notes, now),
        )
        await self._db.commit()
