"""WebSocket endpoints extracted from app.py."""
...
async def websocket_endpoint(websocket: WebSocket):
    await request.app.state.websocket.app.state.ws_manager.connect(websocket)
    try:
        while True:
            data = await websocket.receive_text()
            log.debug("WS received: %s", data)
    except WebSocketDisconnect:
        request.app.state.websocket.app.state.ws_manager.disconnect(websocket)
...
async def chat_stream(message: str = "", session_id: str | None = None):
    ...
    if websocket.app.state.chat_service is None:
    ...
    async def event_generator():
        ...
        task = asyncio.create_task(
            websocket.app.state.chat_service.handle_message(session_id or None, message, progress_queue=progress_queue)
        )
        try:
            while True:
